...
public class SecurityConfig {
	...
	//
	// application.yml 로 하면 SpringDocProperties에서 값을 읽어와야함.
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests(authorize -> authorize
				.requestMatchers(HttpMethod.OPTIONS, "/**").permitAll() // 가장 앞에 있어야함
	...
	CorsConfigurationSource corsConfigurationSource() {
		...
		// 프론트 주소 등록. "http://localhost" 는 웹서버 기본포트 80 생략.
		config.setAllowedOrigins(List.of(corsAllowedOrigin, corsAllowedOrigin + ":5173"));
...
// 토큰 인증 : 리프레시 토큰 , 로그아웃 블랙리스트 , 추가적인 서버 구성(Redis) ... 등의 추가적인 구현 내용이 필요합니다
// -> 토큰 유효성 관리하는 방법을......
// ㄴ> 팀 프로젝트에서는 제외